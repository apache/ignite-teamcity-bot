...
Use a separate clean checkout. Set `PR_REF` only for PR checks. The heavyweight persistent-storage integration tests
are intentionally commented out: uncomment them when the local PR check must also cover legacy Ignite storage and
migration recovery.
...
rem call gradlew.bat :migrator:integrationTest --no-daemon || exit /b 1

...
public class FatBuildCompacted extends BuildRefCompacted implements IVersionedEntity {
    ...
    public FatBuildCompacted(IStringCompactor compactor, Build build) {
        ...
        if (trigXml != null) {
            this.triggered = new TriggeredCompacted();

            this.triggered.type = compactor.getStringId(trigXml.getType());

            final User trigXmlUser = trigXml.getUser();

            if (trigXmlUser != null) {
                this.triggered.userId = Integer.valueOf(trigXmlUser.id);
                this.triggered.userUsername = compactor.getStringId(trigXmlUser.username);
            } else {
                this.triggered.userId = -1;
                this.triggered.userUsername = -1;
            }

            final BuildRef trigBuildRef = trigXml.getBuild();

            this.triggered.buildId = trigBuildRef != null ? trigBuildRef.getId() : -1;
        }
    }
